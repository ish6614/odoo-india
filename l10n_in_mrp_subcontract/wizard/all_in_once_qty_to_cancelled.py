...
class all_process_moves_cancel(osv.osv_memory):
    _name = "all.process.moves.cancel"
    _description = "All In Once Cancel"
    _columns = {
        'select':fields.boolean('Select'),
        'wizard_id': fields.many2one('all.in.once.qty.to.cancelled', 'WorkOrder Move'),
...
all_process_moves_cancel()

class all_in_once_qty_to_cancelled(osv.osv_memory):
    _name = "all.in.once.qty.to.cancelled"
    _description = "All In Once Cancel Wizard"
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        context = context or {}
        res = {}
        all_process_moves_cancel_ids = context and context.get('all_process_moves_cancel_ids', False) or False
        next_stage_workorder_id = context and context.get('next_stage_workorder_id', False) or False

        if 'all_process_moves_cancel_ids' in fields:
            res.update({'all_process_moves_cancel_ids': all_process_moves_cancel_ids})
    ...
    _columns = {
        'all_process_moves_cancel_ids': fields.one2many('all.process.moves.cancel', 'wizard_id', 'All Process Moves'),
    ...
    def to_all_cancel_qty(self, cr, uid, ids, context=None):
        """
        - Process
        ...
        go_to_cancelled = []
        updt_prcs_mve = {}
        for data in wizard_rec.all_process_moves_cancel_ids:
            go_to_cancelled.append(data.process_move_id.id)
